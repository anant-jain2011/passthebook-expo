...
export default function ProfileScreen() {
    ...
    useEffect(() => {
        user.unsafeMetadata?.booksets.length > 0 && (async () => {
            let res = await fetch("https://ptb-backend.vercel.app/get-books?id=" + user.unsafeMetadata?.booksets.join("+"));
            let data = await res.json();
            setBooksets(data);
        })();
    }, []);

    const addBookset = async (booksets) => {
        try {
            // Update the user's unsafe metadata
            await user.update({
                unsafeMetadata: {
            ...
            console.log('Booksets updated successfully!');
        } catch (error) {
            console.error('Failed to update booksets:', error);
        }
    };
        ...
        <SafeAreaView style={[styles.container, { paddingBottom: -insets.bottom }]}>
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.profileSection}>
                    ...
                    <Text style={styles.handle}>{user.emailAddresses[0].emailAddress}</Text>
                    <Text style={styles.bio}>Book lover | Coffee addict | Always reading 📚</Text>

                    {/* Stats */}
                    <View style={styles.statsContainer}>
                        <View style={styles.statItem}>
                            <Text style={styles.statNumber}>{user.unsafeMetadata?.booksets?.length || 0}</Text>
                        ...
                        Booksets.map((bookset, index) => (
                            <View style={styles.bookCard} key={index}>
                                ...
                                <View style={styles.bookInfo}>
                                    ...
                                    <Text style={styles.bookAuthor}>Grade: {bookset.grade}</Text>
                            ...
                            <Text style={{ color: '#999', fontStyle: 'italic', textAlign: "center" }}>No booksets uploaded yet.</Text>
                        )}
                </View>
